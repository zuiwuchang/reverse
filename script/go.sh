...
if [[ "$debug" == 1 ]];then
    ...
    args=(
        go build 
        -ldflags "\"-s -w -X 'github.com/zuiwuchang/reverse/version.Version=$Version' -X 'github.com/zuiwuchang/reverse/version.Date=$date' -X 'github.com/zuiwuchang/reverse/version.Commit=$commit'\""
        -o "bin/$target"
    )
fi
